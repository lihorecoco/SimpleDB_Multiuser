...
public class CreateStudentDB {
	...
	public static void main(String[] args) {
		...
		try {
			...
			stmt.executeUpdate(s,user);
			System.out.println("Table STUDENT created.");
			
			
			s = "insert into STUDENT(SId, SName, MajorId, GradYear) values ";
			String[] studvals = {"(1, 'iso', 10, 2004)",
								 "(2, 'cano', 20, 2004)",
								 "(3, 'caro', 10, 2005)",
								 "(4, 'kýro', 20, 2005)",
								 "(5, 'bob', 30, 2003)",
			...
			for (int i=0; i<studvals.length; i++)
				stmt.executeUpdate(s + studvals[i],user);
			...
			for (int i=0; i<deptvals.length; i++)
				stmt.executeUpdate(s + deptvals[i],user);
			...
			for (int i=0; i<coursevals.length; i++)
				stmt.executeUpdate(s + coursevals[i],user);
			...
			for (int i=0; i<sectvals.length; i++)
				stmt.executeUpdate(s + sectvals[i],user);
			...
			for (int i=0; i<enrollvals.length; i++)
				stmt.executeUpdate(s + enrollvals[i],user);
